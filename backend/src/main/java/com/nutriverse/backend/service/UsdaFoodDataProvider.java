...
public class UsdaFoodDataProvider implements NutritionProvider {
    private static final Logger log = LoggerFactory.getLogger(UsdaFoodDataProvider.class);
    ...
    public UsdaFoodDataProvider(
        ...
        this(createClient(apiUrl), apiKey, dietaryEngine);
    }

    UsdaFoodDataProvider(String apiUrl, String apiKey) {
        this(createClient(apiUrl), apiKey, new DietaryComplianceEngine());
    }

    UsdaFoodDataProvider(RestClient client, String apiKey) {
        this(client, apiKey, new DietaryComplianceEngine());
    }
    ...
    public List<NutritionResult> search(String query) {
        ...
        try {
            ...
            for (Object object : foods) {
                if (!(object instanceof Map<?, ?> food)) continue;
                NutritionResult result = convertFood(food);
                if (result != null && ids.add(result.getSourceId())) results.add(result);
            }
            return results;
        } catch (Exception e) {
    ...
    public NutritionResult findBySourceId(String sourceId) {
        ...
        try {
            Map<?, ?> food = client.get()
                    .uri(uri -> uri.path("/food/{id}")
                            .queryParam("api_key", "{key}")
                            .build(id, apiKey))
                    .retrieve().body(Map.class);

            if (food == null) return null;
            Long returnedId = positiveLong(text(food.get("fdcId")));
    ...
    private NutritionResult convertFood(Map<?, ?> food) {
        ...
        String sourceId = text(food.get("fdcId"));
        String dataType = text(food.get("dataType"));
        if (name == null || positiveLong(sourceId) == null || !supportedType(dataType)
                || !(food.get("foodNutrients") instanceof List<?> nutrients)) return null;

        NutritionResult result = new NutritionResult();
        result.setFoodName(name);

        Double standard = nutrient(nutrients, "208", "Energy", "kcal");
        Double specific = nutrient(nutrients, "958", "Energy (Atwater Specific Factors)", "kcal");
        Double general = nutrient(nutrients, "957", "Energy (Atwater General Factors)", "kcal");
        result.setCalories("Foundation".equalsIgnoreCase(dataType)
                ? first(specific, general, standard)
                : first(standard, specific, general));
        ...
        if (!hasUsefulNutrition(result)) return null;
    ...
    private Double nutrient(
        ...
        for (Object object : nutrients) {
            if (!(object instanceof Map<?, ?> row)) continue;
            Map<?, ?> details = row.get("nutrient") instanceof Map<?, ?> nested ? nested : row;
            String number = firstText(details.get("number"), row.get("nutrientNumber"));
            String name = firstText(details.get("name"), row.get("nutrientName"));
            String unit = firstText(details.get("unitName"), row.get("unitName"));
            boolean matches = expectedNumber.equals(number)
                    || (number == null && expectedName.equalsIgnoreCase(name));
            if (!matches || unit == null || !expectedUnit.equalsIgnoreCase(unit)) continue;
    ...
    private boolean hasUsefulNutrition(NutritionResult result) {
        return result.getCalories() != null || result.getProtein() != null
    ...
    private Double number(Object value) {
        try {
            if (value == null) return null;
            double result = value instanceof Number n
                    ? n.doubleValue() : Double.parseDouble(value.toString());
    ...
    private Long positiveLong(String value) {
        try {
            long result = Long.parseLong(value == null ? "" : value.trim());
    ...
    private ResponseStatusException unavailable(String operation, Exception error) {
        String status = error instanceof RestClientResponseException response
                ? String.valueOf(response.getStatusCode().value()) : "unavailable";
        log.warn("USDA {} failed: status={} type={}",
                operation, status, error.getClass().getSimpleName());
