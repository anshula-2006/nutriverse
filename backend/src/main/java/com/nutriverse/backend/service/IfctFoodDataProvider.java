...
public class IfctFoodDataProvider implements NutritionProvider {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(IfctFoodDataProvider.class);
    ...
    boolean matchesIndexedAlias(String sourceId, String query) {
        ...
        return food != null && food.matchesIndexedAlias(normalizeIfctQuery(query));
    }
    ...
    private void loadCore() throws Exception {
        readCsv("ifct/core.csv", row -> {
            ...
            if (energyKj != null) food.calories = round(energyKj / 4.18);
        });
    }
    ...
    private void readCsv(String path, RowConsumer consumer) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.isBlank()) consumer.accept(parseCsv(line));
            }
        }
    }
    ...
    private static class IfctFood {
        ...
        private String name, scientificName, group, description;
        private Double calories, protein, carbs, fat, fiber;
        private Double iron, calcium, sodium, potassium;
        ...
        private void addAlias(String alias) {
            ...
            String base = alias.replaceFirst("\\s*\\([^)]*\\)\\s*$", "");
            if (!base.isBlank()) indexedAliases.add(normalize(base));
        }
        ...
        private int score(String query) {
            ...
            if (nameValue.equals(query)) return 100;
            if (indexedAliases.contains(query)) return 95;
            if (nameValue.startsWith(query)) return 80;
            if (nameValue.contains(query)) return 60;
            for (String alias : indexedAliases) {
                if (alias.startsWith(query)) return 55;
                if (alias.contains(query)) return 45;
            }
            String all = searchable();
            if (all.contains(query)) return 40;
            for (String word : query.split(" ")) {
                if (word.length() >= 3 && all.contains(word)) return 20;
            }
            return 0;
        }
        private String searchable() {
            return normalize(
                    safe(name) + " " + safe(scientificName) + " "
                            + safe(group) + " " + safe(description) + " "
                            + String.join(" ", indexedAliases)
            );
        }
        private boolean hasNutrition() {
            return calories != null || protein != null || carbs != null
                    || fat != null || fiber != null;
        }
