...
public class ProfileExtractionService {
    private static final Pattern DISLIKE = Pattern.compile(
            "\\b(?:i don't like|i do not like|i dislike|i hate|i don't eat|i do not eat|i don't consume|i do not consume|i cannot eat|i can't eat|i cannot have|i can't have|avoid|exclude)\\s+([^.!?]{1,100})",
    ...
    public ProfileExtractionService(NutritionProfileRepository profileRepository,
    ...
    public void processUserMessage(String userId, String message) {
        ...
        Double height = measurement(text, "height", "cm", 50, 250,
        ...
        Double weight = measurement(text, "weight", "kg", 10, 500,
        ...
        String gender = choice(text, "(?:my gender is|i am|i'm)\\s+(?:a\\s+)?",
                Map.of("female", "FEMALE", "woman", "FEMALE", "male", "MALE",
                        "man", "MALE", "other", "OTHER", "non binary", "OTHER",
                        "non-binary", "OTHER"));
        if (gender != null) {
        ...
        String diet = choice(text, "(?:my diet is|i am|i'm)\\s+(?:a\\s+)?",
                Map.of("vegetarian", "VEGETARIAN", "veg", "VEGETARIAN",
                        "vegan", "VEGAN", "non vegetarian", "NON_VEGETARIAN",
        ...
        String activity = choice(text, "(?:my activity level is|i am|i'm)\\s+",
                Map.of("sedentary", "SEDENTARY", "lightly active", "LIGHTLY_ACTIVE",
        ...
        String goal = choice(text, "(?:my goal is to|my goal is|i want to|i would like to)\\s+",
                Map.of("lose weight", "WEIGHT_LOSS", "gain weight", "WEIGHT_GAIN",
        ...
        if (declaresGlutenRestriction(text) && !"GLUTEN_FREE".equals(profile.getDietaryRestriction())) {
    ...
    public void clearPendingState(String userId) {
        if (userId != null) awaitingFields.remove(userId);
    }

    private boolean declaresGlutenRestriction(String text) {
        if (text.contains("don't have celiac") || text.contains("do not have celiac")
                || text.contains("not celiac")) return false;
        return text.contains("i have celiac") || text.contains("i have coeliac")
                || text.contains("celiac disease") || text.contains("coeliac disease")
                || text.contains("gluten free") || text.contains("gluten-free")
                || text.contains("can't eat gluten") || text.contains("cannot eat gluten")
                || text.contains("can't have gluten") || text.contains("cannot have gluten")
    ...
    private List<String> extractDislikes(String text) {
        ...
        for (String part : value.split("\\s*(?:,|\\bor\\b|\\band\\b)\\s*")) {
            ...
            if ("gluten".equals(food) || food.contains("celiac") || food.contains("coeliac")) continue;
            result.add(food);
        }
        return result;
    }
    ...
    private Integer age(String text, boolean awaited) {
        ...
        if (value == null) value = match(text,
                "\\bmy age(?: is|:)?\\s+(\\d{1,3})(?![\\d.])\\b");
        if (value == null && awaited) value = match(text,
                "^(?:i am |i'm )?(\\d{1,3})(?: years? old)?[.!]?$");
        if (value == null) return null;
        int age = Integer.parseInt(value);
        return age >= 1 && age <= 120 ? age : null;
    }

    private Double measurement(String text, String field, String unit,
                               double min, double max, boolean awaited) {
    ...
    private String exerciseActivity(String text, boolean awaited) {
        String days = match(text,
                "\\bi (?:exercise|work out|workout|train)\\s+([0-7])\\s*(?:days?|times?) (?:a|per) week\\b");
        if (days == null && awaited) days = match(text,
    ...
    private String normalize(String text) {
        return text.toLowerCase(Locale.ROOT).trim().replace('\u2019', '\'').replace('\u2011', '-');
    }
}
